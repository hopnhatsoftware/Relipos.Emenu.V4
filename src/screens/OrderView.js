...
import {
  AppState, TouchableOpacity, Dimensions, TouchableHighlight, Image, ActivityIndicator, UIManager, StatusBar, ImageBackground, Keyboard, StyleSheet, Alert, Platform, Animated, Easing, Text, View,
...
import { GetViewGroup, GetPrdChildGroups, getProductByGroup, getTicketInforOnTable, sendOrder, CheckAndGetOrder, CheckAndGetOrder2, SetMenu_getChoiceCategory, getByChoiceId, CancelOrder, CallServices, getLanguage } from "../services";
        ...
        if (table.OrderId == null) {
          CheckAndGetOrder(table, OrdPlatform).then(res => {
            if (res.Status == 1) {
              ...
              CheckAndGetOrder2(table, OrdPlatform).then(res => {
                if (res.Status == 1) {
                  table.OrderId = res.Data;
                }
                else {
      ...
      await sendOrder(Config, table, OrdPlatform, CartInfor.items).then(async res => {
        if (res.Status == 1) {
          await _remove("APP@CART", async () => {
            await this.setState({ CartInfor: { TotalQuantity: 0, TotalAmount: 0, ItemAmount: 0, items: [], } }, async () => {
              ...
              CheckAndGetOrder2(table, OrdPlatform).then(res => {
                if (res.Status == 1) {
                  table.OrderId = res.Data;
                 _storeData("APP@TABLE", JSON.stringify(table), async () => {
                  _storeData('APP@TimeToNextBooking', JSON.stringify(new Date().getTime()), async () => {
                    await this.setState({
                      table: table, isShowMash: false,
                      TimeToNextBooking: Config.I_Limit_Booking_Time ? Config.I_Limit_Booking_Time : 5
          ...
          Alert.alert(this.translate.Get("Thông báo"), res.Exception_Message, [
            {
              text: "OK", onPress: () => { this.setState({ isShowMash: false }) }
            }
          ]);
        }
      }).catch(error => {
            ...
            <TouchableOpacity name='dvImage' style={{ flexDirection: "row", width: '100%', height: '100%' }}
              ...
              <Image resizeMode="contain"
                ...
                source={item.PrdImageUrl ? {
                  uri: this.state.endpoint + "/Resources/Images/Product/" + item.PrdImageUrl
      ...
      <View style={{ flex: 1, backgroundColor: isColor == true ? '#333333' : "#DDDDDD" }}>
          ...
          <Animated.View style={[styles.BottonMenu, { width: '83%', height: '7%' }]}>
            {this.state.ShowFullCart ?
              <View style={{ width: "100%", flexDirection: "row", height: '100%' }}>
                ...
                <TouchableOpacity onPress={() => { this.CartToggleHandle(true) }} style={{
                  width: '20%', height: '100%', flexDirection: "row", color: "white", alignItems: "center", justifyContent: "center",
        ...
        }
        {
          ...
          (CartItemSelected != undefined && CartItemSelected != null && CartProductIndex >= 0 && CartItemSelected.PrdIsSetMenu && this.state.showSetInCart) ? (
            <View name='pnListSetmenuView' style={{
              width: SCREEN_WIDTH, height: SCREEN_HEIGHT, backgroundColor: "rgba(98,98,98,0.6)",
              position: "absolute",
              ...
              <View style={[{
                ...
                <View name='pnShowSetHeader' style={{
                  flexDirection: "row", justifyContent: "center", alignItems: "center", borderTopLeftRadius: 4,
                ...
                <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center", height: SCREEN_HEIGHT * 0.07 }} >
                  ...
                  <TouchableOpacity onPress={() => this.setState({ showSetInCart: false, CartItemSelected: null, CartProductIndex: -1, })}>
                    <View style={[{
                      width: (Center.width / 5), height: '99%', flexDirection: "row", color: "white", alignItems: "center", justifyContent: "center", backgroundColor: "#008bc5",
                  ...
                  <TouchableOpacity onPress={() => this._SetMenuViewOpen(CartItemSelected, CartProductIndex)}>
                    <View style={[{
                      width: (Center.width / 5), height: '99%', marginLeft: 10, flexDirection: "row", color: "white",
