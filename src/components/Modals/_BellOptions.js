...
export class _BellOptions extends React.Component {
          ...
          <View style={{ width: "65%", alignItems: "flex-start", flexDirection: 'column', padding: FontSize * 0.3, justifyContent: 'center' }}>
            <View style={{ paddingBottom: FontSize * 0.1, flexDirection: 'column' }}>
              <View style={{ flexDirection: 'row' }}>
                <Text style={{ fontSize: ITEM_FONT_SIZE * 1.5, fontWeight: 'bold' }}>{t._("Relipos")} </Text>
                <Text style={{ fontSize: ITEM_FONT_SIZE * 1.5, color: colors.grey3 }}>{t._("menu")}</Text>
              </View>
              <Text style={{ paddingLeft: FontSize, paddingTop: FontSize * 0.1, fontSize: ITEM_FONT_SIZE * 0.8, color: colors.grey3 }}>{t._("Version")}: {manifest.version} - {t._("Build")}: {Platform.OS == 'ios' ? manifest.ios.buildNumber : manifest.android.versionCode}</Text>
            </View>
            {'S_DepartmentName' in settings && settings.S_DepartmentName != '' ?
              <Text style={{ paddingTop: FontSize * 0.4, fontSize: FontSize, fontWeight: 'bold' }}>{t._('Restaurant') + ' ' + settings.S_DepartmentName}</Text>
              : <Text style={{ paddingTop: FontSize * 0.4, fontSize: FontSize, fontWeight: 'bold' }}>{t._("Hop Nhat Software")}</Text>}
            <View style={{ flexDirection: 'row', paddingTop: FontSize * 0.4, }}>
              <Text style={{ fontSize: FontSize, fontWeight: 'bold' }}>{t._("Support platform:")} </Text>
              <Text style={{ fontSize: FontSize, color: colors.grey3 }}>{t._("Android/IOS")}</Text>
            </View>
            <View style={{ paddingBottom: FontSize * 0.2, flexDirection: 'column' }}>
              <Text style={{ paddingTop: FontSize * 0.4, fontSize: FontSize, fontWeight: 'bold' }}>{t._("Product features: ")}</Text>
              <Text style={{ paddingLeft: FontSize * 0.2, paddingTop: FontSize * 0.2, fontSize: FontSize, color: colors.grey3 }}>- {t._("Customers take the initiative of ordering.")}</Text>
              <Text style={{ paddingLeft: FontSize * 0.2, paddingTop: FontSize * 0.2, fontSize: FontSize, color: colors.grey3 }}>- {t._("Reviews ordered.")}</Text>
              <Text style={{ paddingLeft: FontSize * 0.2, paddingTop: FontSize * 0.2, fontSize: FontSize, color: colors.grey3 }}>- {t._("Easy to order more items.")}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {
              language != 1 ?
                <TouchableOpacity style={{ paddingLeft: 10, paddingRight: 5, paddingTop: 2, justifyContent: 'center', alignItems: 'center', }}
                  onPress={() => {changeLanguage(1); setState({ showFilterBell: false }); }}>
                ...
                :
                <TouchableOpacity style={{ paddingLeft: 10, paddingRight: 5, paddingTop: 2, justifyContent: 'center', alignItems: 'center', }}
                  onPress={() => {changeLanguage(2); setState({ showFilterBell: false }); }}>
            ...
            {/* <FlatList
                data={[
                  // { ID: 2, text: t._("Call for services") },
                  // { ID: 1, text: t._("Call for payment") },
                  language != 1
                    ? {
                      ID: 4,
                      text: t._("Tiếng Việt"),
